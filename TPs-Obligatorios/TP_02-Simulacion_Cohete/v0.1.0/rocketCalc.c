...
void rocketCalc(int minVel, int minH, int numRocket, int *rocketSucc, struct Rocket *rockTemp){
	...
	for(i = 0; i < 3; i++){
		...
		rockTemp -> RandAngleRAD[i] = rockTemp -> RandAngle[i] * (float) M_PI / (float) 180;
		
		// math.h calcula en radianes:
		rockTemp -> CosAngle = (float) cos(rockTemp -> RandAngleRAD[i]);
		rockTemp -> SinAngle = (float) sin(rockTemp -> RandAngleRAD[i]);
		
		// Cálculo de velocidad y altura:
		// ### REVISAR FÓRMULAS ###
		rockTemp -> Velocity[i] = (rockTemp -> CosAngle) * (float) (rockTemp -> RandForce[i]) / (float) (rockTemp -> Mass);
		rockTemp -> Height[i] = (rockTemp -> SinAngle) * (float) (rockTemp -> RandForce[i]) / (float) (rockTemp -> Mass);
...
*/
	...
	"\nIntento:                     [ %d ]\n", i+1);
		printf(
	"\nÁngulo de lanzamiento %d:     [ %d ]", rockData.Attempt, rockData.RandAngle[i]);
		printf(
	"\nFuerza de empuje %d:          [ %d ]", rockData.Attempt, rockData.RandForce[i]);
		printf(                                                        
	"\nVelocidad inicial %d:         [ %d ]", rockData.Attempt, rockData.Velocity[i]);
		printf(                                                        
	"\nAltura inicial %d:            [ %d ]", rockData.Attempt, rockData.Height[i]);
		printf(
	"\nEnergía Cinética %d:          [ %.2f ]", rockData.Attempt, rockData.eC[i]);
		printf(                                                          
	"\nEnergía Potencial %d:         [ %.2f ]", rockData.Attempt, rockData.eP[i]);
		printf(                                                          
	"\nRendimiento %d:               [ %s ]\n", rockData.Attempt, rockData.PerfMes[i]);	
	}
